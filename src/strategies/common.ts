...
export function buildMarketView(bot: BotConfig, quote: Quote): MarketView {
...
export function planMakerQuote(params: PlanQuoteParams): StrategyAction[] {
  ...
  const staleOrOffMarket = sameSideOrders.filter((order) =>
...
export function sizeForMaker(bot: BotConfig, balance: Balance, price: string): string {
  return maxAffordableBuySize(bot.maxOrderSize, balance.availableUSDC, price);
}

export function sizeForTaker(bot: BotConfig, balance: Balance, price: string): string {
...
function shouldCancelWorkingOrder(params: {
  ...
  if (params.order.createdAt) {
    ...
    if (ageMs >= params.bot.staleOrderMs) {
      return true;
    }
  }
  ...
  return fairDistanceTicks > params.bot.staleDistanceTicks + params.bot.targetSpreadTicks;
}
