...
import { PolyApiError } from "../dist/api/apiClient.js";
import { BotRiskManager } from "../dist/runner/botRiskManager.js";
...
function createBotConfig(overrides = {}) {
  ...
  return {
    ...base,
    ...overrides,
    dynamicMarketMaker: { ...base.dynamicMarketMaker, ...(overrides.dynamicMarketMaker ?? {}) },
    risk: { ...base.risk, ...(overrides.risk ?? {}) },
  };
}

function createManager(bot) {
...
function createQuoteResponse() {
  return {
    marketId: "market-1",
    quotes: [
      { outcomeId: "yes", outcomeName: "YES", bestBid: "0.49", bestAsk: "0.51", midPrice: "0.50", lastPrice: "0.50", lastTradeAt: null },
      { outcomeId: "no", outcomeName: "NO", bestBid: "0.49", bestAsk: "0.51", midPrice: "0.50", lastPrice: "0.50", lastTradeAt: null },
    ],
  };
}

{
  ...
  const evaluation = await manager.evaluateMarket({
    ...
    freshness: { marketStateAgeMs: 20000, accountStateAgeMs: 20000, staleStateDetectedCount: 1 },
  });
...
{
  const manager = createManager(createBotConfig({ risk: { maxCapitalPerMarketCents: 50 } }));
  const evaluation = await manager.evaluateMarket({
    marketId: "market-1",
    balance: createBalance(),
    positions: createPositions(),
    openOrders: [],
    quoteResponse: createQuoteResponse(),
    freshness: { marketStateAgeMs: 1000, accountStateAgeMs: 1000, staleStateDetectedCount: 0 },
  });
  const decision = manager.checkPlacement({
    marketId: "market-1",
    outcomeId: "yes",
    action: { type: "place", reason: "test", side: "BUY", marketId: "market-1", outcomeId: "yes", price: "0.60", size: "1.000000", idempotencyKey: "id", clientOrderId: "id" },
    balance: createBalance(),
    positions: createPositions(),
    openOrders: [],
